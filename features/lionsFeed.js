 ...
 */
async function createLionsFeedSheet(lionsFeedData) {
  const sheet = SHEETS.LIONS_FEED;
  lionsFeedData.sort((a, b) => a[1].localeCompare(b[1]));

  await clearAndAppendHeaders(sheet, HEADERS.LIONS_FEED);
  await writeDataToSheet(sheet, lionsFeedData);
  await applyAlternatingRowStyles(sheet);

  const CELL_REFERENCE = "G1";
  const FONT_SIZE = 16;
  const IS_BOLD = true;
  const value = "Total Count: " + lionsFeedData.length;
  await appendValueToCell(sheet, value, CELL_REFERENCE, FONT_SIZE, IS_BOLD);
}
